    ...
    if (data.length) {
        ...
        for (const artist of artists) {
            let html = `<div class="${bigCardLayout}">
                <a class="big-card artist" href="artist.html?artist_id=${artist.id}">
        ...
        for (const song of fourSongs) {
            let html = `<div class="song">
                ...
                <div class="details">
                    <h4>${song.title}</h4>
                    <a href="artist.html?artist_id=${song.artist.id}">${song.artist.name}</a>
                </div>
            </div>`;
        ...
        $(".showcase .details a").attr(
            "href",
            `artist.html?artist_id=${featuredSong.artist.id}`
        );
    }
};
